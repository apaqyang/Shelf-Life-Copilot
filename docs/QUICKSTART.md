...
- `monthly_report_customerA.pdf` / `monthly_report_customerB.pdf` — 真实月度报告 PDF（销售弹药）
...
跟着 8 个问题答一遍，**不到 1 分钟出来一份给老板看的 ROI 一页纸 PDF**，
落到 `data/leads/<你的厂名>_<日期>.pdf`。
...
A: 两条路：
1. **5 分钟现场版**：跟着 [docs/DEMO_SCRIPT.md](DEMO_SCRIPT.md) 念，对着 `make demo` 输出的卡片样本讲
2. **2 分钟兜底视频版**：[docs/FALLBACK_VIDEO.md](FALLBACK_VIDEO.md) 是分镜脚本，给你一个录制蓝本
...
- 还有疑问：GitHub Issues 提一个，或者 [PRD.md](PRD.md) 找答案
